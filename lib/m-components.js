...
var ImageUploader = function () {
  function ImageUploader(imageUploader) {
  ...
  _createClass(ImageUploader, [{
    ...
    value: function handleInputChange(e) {
      ...
      if (file) {
        this.readThumbnail(file).then(function (data) {
          ...
          _this.uploadingIndex = _this.fileList.length - 1;
          e.target.value = '';
          _this.renderThumbnail();
          _this.uploadFile(file, _this.fileList.length - 1);
        });
      }
    }
  }, {
    ...
    value: function uploadFile(file, index) {
      ...
      var handleProgress = function handleProgress(e) {
        if (e.lengthComputable) {
          _this2.itemDoms[index].find('.' + prefix + '-percentage').html(Math.round(e.loaded / e.total * 100) + "%");
        }
      };

      var handleUploadSuccess = function handleUploadSuccess(e) {
        _this2.uploading = false;
        _this2.itemDoms[index].find('.' + prefix + '-percentage').removeClass('active');
      };

      var handleUploadFail = function handleUploadFail(e) {
        _this2.uploading = false;
        _this2.itemDoms[index].find('.' + prefix + '-fail').addClass('active');
        _this2.itemDoms[index].find('.' + prefix + '-percentage').removeClass('active');
      };
      ...
      $.ajax({
        ...
        success: function success(data, status, xhr) {
          _this2.fileList[index].status = 'done';
          handleUploadSuccess();
          console.log(data);
        },
        error: function error(xhr, errorType, _error) {
          _this2.fileList[index].status = 'error';
          handleUploadFail();
          console.log('error');
    ...
    value: function renderThumbnail() {
      ...
      this.itemDoms = this.fileList.map(function (file, index) {
...
$.each(imageUploaders, function (index, imageUploader) {
